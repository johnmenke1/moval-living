// Apply the BestOfNomination.ownerId migration directly to Neon,
// then insert the corresponding _prisma_migrations row so Prisma CLI
// recognizes it as applied. Mirrors the workflow we used for the
// Review.ownerId migration in
// prisma/migrations/20260823023744_add_review_owner_id/* and the
// BestOfVote migration in 20260822162306_add_best_of_vote_*.
...
const indexSql = `
CREATE INDEX IF NOT EXISTS "BestOfNomination_ownerId_idx"
  ON "BestOfNomination"("ownerId");
`
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE conname = 'BestOfNomination_ownerId_fk'
  ) THEN
    ALTER TABLE "BestOfNomination"
...
const migrationName = '20260824030000_add_best_of_nomination_owner_id'
